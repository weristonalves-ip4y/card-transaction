...
const cardWithdrawalMovementTypeID = 24

type CardRepository = purchase.CardRepository
type TransactionRepository = purchase.TransactionRepository
type BalanceRepository = purchase.BalanceRepository
type CardMovementRepository = purchase.CardMovementRepository
type TransactionalContext = purchase.TransactionalContext
type TransactionManager = purchase.TransactionManager

type TransactionOutput = purchase.TransactionOutput

type WithdrawalCardTransaction struct {
...
type WithdrawalTransaction struct {
	cardUseCase WithdrawalCardTransaction
}

func NewWithdrawalCardTransaction(
	cardRepo CardRepository,
	txRepo TransactionRepository,
	balanceRepo BalanceRepository,
	movementRepo CardMovementRepository,
	txManager ...TransactionManager,
) WithdrawalCardTransaction {
	var resolvedTxManager TransactionManager
	if len(txManager) > 0 {
		resolvedTxManager = txManager[0]
	}

	return WithdrawalCardTransaction{
		cardRepo:     cardRepo,
		txRepo:       txRepo,
...
func (a WithdrawalCardTransaction) WithNotificationDispatcher(dispatcher domainnotification.Dispatcher) WithdrawalCardTransaction {
	a.dispatcher = dispatcher
	return a
}

func NewWithdrawalTransaction(
	cardRepo CardRepository,
	txRepo TransactionRepository,
	balanceRepo BalanceRepository,
	movementRepo CardMovementRepository,
	...
	cardUseCase := NewWithdrawalCardTransaction(cardRepo, txRepo, balanceRepo, movementRepo, txManager)
...
func (a WithdrawalTransaction) Execute(input dto.AuthorizeRequest) (TransactionOutput, error) {
	return a.cardUseCase.Execute(input)

}

func (a WithdrawalCardTransaction) Execute(input dto.AuthorizeRequest) (TransactionOutput, error) {
	tx, err := newWithdrawalTransactionFromInput(input)
	if err != nil {
		return TransactionOutput{}, err
	}

	isDuplicate, err := a.txRepo.ExistsByIdentifier(input.WithdrawalID)
	if err != nil {
		_, _ = persistSerializedTransaction(a.txRepo, tx, "96")
		return TransactionOutput{}, fmt.Errorf("checking duplicate: %w", err)
	}
	if isDuplicate {
		output := rejectWithdrawalByCode("07")
	...
	if result := card.ValidateCard(c); !result.Approved {
		output := rejectWithdrawalByCode(result.Code)
		_, _ = persistSerializedTransaction(a.txRepo, tx, output.Code)
		return output, nil
	}
	...
	if card.MonthsLimitExceeded(c, monthlySum, amount, input.ForceAccept) {
		output := rejectWithdrawalByCode("01")
		_, _ = persistSerializedTransaction(a.txRepo, tx, output.Code)
		return output, nil
	}
	...
	approvedOutput := approveWithdrawal()
	var authorizationID int64
	persistenceCompleted := false
	movementDescription := buildCardWithdrawalMovementDescription(input)
	...
	if err != nil {
		if !persistenceCompleted {
			return rejectWithdrawalByCode("96"), nil
		}
		return TransactionOutput{}, err
	}
...
func buildCardWithdrawalMovementDescription(input dto.AuthorizeRequest) string {
	location := input.ResolveEstablishmentLocation()
	...
	return "SAQUE CARTÃO | " + strings.ToUpper(location)
}

func newWithdrawalTransactionFromInput(input dto.AuthorizeRequest) (transaction.Transaction, error) {
	...
	tx = tx.WithPersistenceBase(tx.UUID, input.Card.PaysmartID, nil, input.WithdrawalID)
...
func approveWithdrawal() TransactionOutput {
...
func rejectWithdrawalByCode(code string) TransactionOutput {
